"""Tool to run the apixaban model factory and simulation experiments."""
...
from pkdb_models.models.apixaban import APIXABAN_PATH
...
def _setup_custom_results_paths(results_dir: str):
    ...
    import pkdb_models.models.apixaban as apixaban
    custom_path = Path(results_dir).resolve()
    custom_path.mkdir(parents=True, exist_ok=True)
    # Override the module paths
    apixaban.RESULTS_PATH = custom_path
    apixaban.RESULTS_PATH_SIMULATION = custom_path / "simulation"
...
def _list_available_experiments():
    ...
    console.print('[dim]Example: run_apixaban --action simulate --experiments "misc,LaCreta2016"[/dim]')
    console.print('[dim]Or use "all" to run all experiments: run_apixaban --action simulate --experiments all[/dim]\n')
...
def main() -> None:
    ...
    if options.results_dir:
        ...
        if options.action and options.action.lower() == "factory":
            console.print(f"[cyan]Factory will use model base path: {apixaban.MODEL_BASE_PATH}[/cyan]")
        else:
...
if __name__ == "__main__":
    """
    This script is intended to be run from the command line as 'run_apixaban'.
    ...
    2. Run the Model Factory:
       ...
       $ run_apixaban --action factory

    3. Run Simulations:
       List available experiments:
       $ run_apixaban --action list_experiments

       Run all experiments:
       $ run_apixaban --action simulate --experiments all

    5. Run Everything:
       Runs factory and all simulations.
       $ run_apixaban --action all
       
       With custom results directory for figures:
       $ run_apixaban --action all --results-dir '/path/to/my/results'
    """
    main()